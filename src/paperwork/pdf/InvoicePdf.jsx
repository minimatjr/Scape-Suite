...
export default function InvoicePdf({ data }) {
  ...
  return (
    <Document title={`Invoice-${invoice.number || "Draft"}.pdf`}>
      <Page size="A4" style={S.page}>
        <View style={[S.row, S.header]}>
          ...
          <View style={S.titleBlock}>
            ...
            <View style={S.docMeta}>
              <Text>
                <Text style={S.subtle}>Invoice No: </Text>
                {invoice.number || "—"}
              </Text>
              <Text>
                <Text style={S.subtle}>Invoice Date: </Text>
                {invoice.date || "—"}
              </Text>
              <Text>
                <Text style={S.subtle}>Due Date: </Text>
                {invoice.dueDate || "—"}
              </Text>
              {invoice.poNumber ? (
                <Text>
                  <Text style={S.subtle}>PO: </Text>
                  {invoice.poNumber}
                </Text>
              ) : null}
            </View>
        ...
        <View style={[S.row, { gap: 12 }]}>
          ...
          <View style={[S.card, { flex: 1 }]}>
            <Text style={S.sectionTitle}>Work / Reference</Text>
            <Text>{invoice.projectTitle || "—"}</Text>
            {invoice.siteAddress ? <Text>{invoice.siteAddress}</Text> : null}
            {invoice.reference ? (
              <Text>
          ...
          <View style={S.totals}>
            <View style={S.totalsBox}>
              ...
              <View style={[S.totalsRow, { marginBottom: 0 }]}>
                <Text style={S.totalsStrong}>Total Due</Text>
        ...
        <View style={S.footer}>
          <Text style={S.subtle}>
            {business.footerNote || "Please include the invoice number as a payment reference."}
          </Text>
        </View>
      </Page>
    </Document>
